...
export default function AIProviderDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Mock data for now since services are missing
      setPreferences({
        provider: 'openai',
      ...
      setUsageStats({
        daily: { requests: 45, tokens: 1200 },
        weekly: { requests: 280, tokens: 8500 },
        monthly: { requests: 1250, tokens: 45000 }
      });
    } catch (error) {
    ...
    <div className="min-h-screen flex bg-gradient-to-br from-black via-gray-900 to-green-950">
      ...
      <Dialog open={showSettings} onOpenChange={setShowSettings}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-black/90 border-white/10">
          ...
          <div className="p-4">
            <p className="text-gray-400">AI Settings panel would go here.</p>
          </div>
        </DialogContent>
      </Dialog>
    </div>
