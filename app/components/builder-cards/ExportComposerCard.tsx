...
export function ExportComposerCard() {
  ...
  // Use the tool selected in ValidationCard, fallback to cursor if none selected
  const selectedTool = state.validationQuestions.selectedTool || 'cursor';
      ...
      <Card className="bg-black/40 backdrop-blur-sm border-white/10">
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="w-full h-12 bg-white/10 border-white/20 rounded-md flex items-center px-3">
              <span className="text-lg mr-3">{selectedToolData?.icon}</span>
              <div className="flex-1">
                <div className="font-medium text-sm text-white">{selectedToolData?.name}</div>
                <div className="text-xs text-gray-400 truncate">{selectedToolData?.description}</div>
              </div>
          ...
          {selectedToolData && (
            <div className="mt-4 p-4 bg-gradient-to-r from-green-500/20 to-blue-500/20 backdrop-blur-sm rounded-lg border border-green-500/30">
              ...
              <div className="text-xs text-gray-400 bg-white/5 p-2 rounded">
                💡 <strong>Export Format:</strong> {exportFormat.charAt(0).toUpperCase() + exportFormat.slice(1)} format optimized for {selectedToolData.name} development workflow
              </div>
            </div>
          )}
        </CardContent>
